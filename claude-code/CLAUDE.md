...
| browser | default | Autonomous browser agent: multi-step web interaction via playwright-cli. Spawned by /browse or directly by other agents. |
| chrome | default | Authenticated browser via `claude --chrome -p`. Subprocess with access to user's logged-in Chrome sessions. Spawned by /chrome or as auth escalation from browser agent. |
...
|------|----------|-------------|-------|-------------|
| **WebFetch** | Read a single page's text content | Minimal | No | Yes |
...
| **Claude in Chrome** (`claude --chrome`) | Authenticated workflows on user's actual browser (logged-in sites, cookies, open tabs) | ~7.7% | Yes (user's sessions) | No |
